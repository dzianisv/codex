...
fn find_codex_cli(cwd: &Path) -> Option<PathBuf> {
    // Only trust `cargo_bin("codex")` when Cargo exported the current test-run
    // binary path. Otherwise the assert_cmd fallback can resolve a stale
    // `target/debug/codex` artifact from an older build, which masks current TUI
    // changes in these interactive regressions.
    if cargo_test_exported_bin_path("codex")
        && let Ok(path) = codex_utils_cargo_bin::cargo_bin("codex")
    {
        return Some(path);
    }

    // When Cargo did not export the test-run binary path, build and use a
    // fresh fallback binary once per process so PTY E2E coverage cannot drift
    // onto a stale target artifact.
...
fn cargo_test_exported_bin_path(name: &str) -> bool {
    let env_key = format!("CARGO_BIN_EXE_{name}");
    if std::env::var_os(&env_key).is_some() {
        return true;
    }
    let underscore_env_key = format!("CARGO_BIN_EXE_{}", name.replace('-', "_"));
    std::env::var_os(underscore_env_key).is_some()
}
