...
- `config/mcpServer/reload` — reload MCP server config from disk and reconnect loaded threads immediately; returns `{}`. Use this after editing `config.toml` without restarting the server.
