...
fn run_azure_reflection_test(
    prompt: &str,
) -> (assert_cmd::assert::Assert, TempDir, Vec<u8>, Vec<u8>) {
    ...
    std::fs::write(
        ...
        create_azure_config(&base_url, &model),
    )
    .unwrap();
...
After creating both files, run the test to verify it passes."#;
    ...
    assert!(hello_path.exists(), "hello.py was not created by the model");
