...
fn truncate_string(s: &str, max_len: usize) -> String {
    if s.len() <= max_len {
        ...
        let mut truncate_at = max_len.saturating_sub(3).min(s.len());
...
/// * `model_info` - Model metadata for the judge model
/// * `session_telemetry` - Session telemetry used for request tracing/metrics
...
pub async fn evaluate_reflection(
    ...
    info!(
        ...
        context.attempt, model_info.slug
    );
    ...
    let prompt = Prompt {
        ...
        base_instructions: BaseInstructions {
            text: "You are a task verification judge. Respond only with JSON.".to_string(),
        },
        personality: None,
...
That's my verdict."#;
    ...
    fn test_truncate_string_unicode_boundary() {
        let input = "abc🧪defgh";
        assert_eq!(truncate_string(input, 7), "abc...");
        assert_eq!(truncate_string(input, 8), "abc...");
        assert_eq!(truncate_string(input, 10), "abc🧪...");
    }

    #[test]
    fn test_truncate_string_unicode_boundary_at_500_byte_cutoff() {
        let prefix = "a".repeat(495);
        let input = format!("{prefix}🧪tail");
        let truncated = truncate_string(&input, 500);
    ...
    fn test_output_indicates_error_mixed_with_strong_indicator() {
        ...
        assert!(output_indicates_error(
            "0 errors in file, but Failed: compilation error"
        ));
    }
    ...
    fn test_judge_prompt_includes_all_context() {
        ...
        let items = vec![
            ...
            ResponseItem::FunctionCallOutput {
                call_id: "c2".to_string(),
                output: FunctionCallOutputPayload::from_text("File written".to_string()),
            },
            ResponseItem::Message {
                id: None,
                role: "assistant".to_string(),
